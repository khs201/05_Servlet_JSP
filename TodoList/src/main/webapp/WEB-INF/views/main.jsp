...
<body>
	...
	<table border="1" style="border-collapse: collapse;">
		...
		<tbody>
			<!-- c:forEach == JST의 향상된 for문 
		 items : 순차 접근할 List, 배열 
		 var : 하나씩 꺼내서 저장할 변수 이름 지정
		 varStatus : 반복 상태 관련 객체(몇 번 인덱스, 첫바퀴? 마지막? 등등)
-->
     ...
     c:if (조건문, else 없음)
		 ...
		 not == ! (논리 부정 연산자)
		 --> <c:if test="${todo.complete}">O</c:if> <c:if
							test="${not todo.complete}">X</c:if>
					</th>
					<td>${todo.regDate}</td>
				</tr>
	...
	<!-- 
		 ...
		 Java가 해석 우선순위가 높다! (먼저 해석된다!)
-->

	<!-- session scope에 message 속성이 비어있지 않다면 실행한다는 조건문 -->
